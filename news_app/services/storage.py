...
DEFAULT_CLASS_CURRENT = {
    ...
    "script_es": "",
    "script_es_pairs": [],
    "translation_scaffolding_enabled": False,
    ...
    current.update(
        {
            ...
            "translation_scaffolding_enabled": bool(raw.get("translation_scaffolding_enabled", False)),
        ...
        normalized_archive.append(
            {
                ...
                "translation_scaffolding_enabled": bool(item.get("translation_scaffolding_enabled", False)),
...
def _migrate_legacy_state(data: dict) -> dict:
    ...
    for key in ("display_language", "ai_model", "ai_transcript_model", "ai_eval_model", "openai_api_key", "active_class_id"):
...
def update_settings(**kwargs) -> dict:
    state = load_state()
    for key in ("ai_model", "ai_transcript_model", "ai_eval_model", "openai_api_key"):
        if key in kwargs:
